  ...
  const updatePriceDisplay = useCallback((params: any) => {
    if (params && params.value) {
      ...
      if (lastDataPoint) {
        ...
        setPriceChange(((lastDataPoint.price - priceData[0].price) / priceData[0].price) * 100);
      }
    }
  }, [priceData]);
    ...
    if (chart) {
      chart.on('updateAxisPointer', (params: any) => {
        if (params.axesInfo && params.axesInfo.length > 0) {
          ...
          const pointIndex = priceData.findIndex(point => point.timestamp === axisValue);
          if (pointIndex !== -1) {
            updatePriceDisplay({ value: [axisValue, priceData[pointIndex].price] });
          }
        } else {
